...
def MatrixFilling_opt(mesh,rooftops):
    try:
        ...
        ha_m = ha_m.transpose()
        # 标准高斯点
        W_x,P_x = getWeightAndPoint_S(Rule_Order_Guassion)
        W_y,P_y = getWeightAndPoint_S(Rule_Order_Guassion)
    ...
    for n,edge_n in enumerate(rooftops): # 遍历所有测试
        ...
        try:
            ...
            # 计算格林函数　[高斯点个数，区间段]
            ejkR_R_pos = np.divide(np.exp(-jK*DDist_pos),DDist_pos)
            ejkR_R_neg = np.divide(np.exp(-jK*DDist_neg),DDist_neg)
            # 计算格林函数的数值积分 [区间段]
            ...
            Int_ejkR_R_pos = np.multiply(np.sum(ejkR_RXw_pos,axis=0), half_lm_am)# 0.5l_m 和0.5a_m是高斯积分权重的缩放因子
            Int_ejkR_R_neg = np.multiply(np.sum(ejkR_RXw_neg,axis=0), half_lm_am)
            # 计算x_ejkR_RXw　用于＋矩形
            x_ejkR_RXw_pos = np.multiply((PP_x+1.).transpose(),ejkR_RXw_pos)
            x_ejkR_RXw_neg = np.multiply((PP_x+1.).transpose(),ejkR_RXw_neg)
            Int_x_ejkR_RXw_pos = np.multiply(np.sum(x_ejkR_RXw_pos,axis=0), half_lm__2_am)# 0.25l_m是权重缩放因子和位置缩放因子的乘积
            Int_x_ejkR_RXw_neg = np.multiply(np.sum(x_ejkR_RXw_neg,axis=0), half_lm__2_am)
...
def MatrixFilling_opt_new(mesh,rooftops):
    ...
    for n,edge_n in enumerate(rooftops): # 遍历所有测试
        ...
        try:
            ...
            ejkR_R_pos = np.multiply(ejkR0_pos,np.divide((1-jK*(DDist_pos-DDist0_pos)),DDist_pos))
            ejkR_R_neg = np.multiply(ejkR0_neg,np.divide((1-jK*(DDist_neg-DDist0_neg)),DDist_neg))
            # 计算格林函数的数值积分 [区间段]
            ejkR_RXw_pos = np.multiply(np.multiply(WW_x.transpose(),WW_y.transpose()),ejkR_R_pos)
            ejkR_RXw_neg = np.multiply(np.multiply(WW_x.transpose(),WW_y.transpose()),ejkR_R_neg)
            Int_ejkR_R_pos = np.multiply(np.sum(ejkR_RXw_pos,axis=0), half_lm_am)# 0.5l_m 和0.5a_m是高斯积分权重的缩放因子
...
def MatrixFilling_opt_new_full(mesh,rooftops):
    try:
        # 矩阵化的x0_m　[坐标，区间段]
        x0_m = np.array([meshcell[0] for meshcell in mesh]).transpose()
        # 矩阵化的ｌｍ　[区间段]
        ...
        ha_m = ha_m.transpose()
        # 标准高斯点
        W_x,P_x = getWeightAndPoint_S(2)
        W_y,P_y = getWeightAndPoint_S(1)
        # 存储每个矩形上的高斯点 [高斯点个数，坐标，区间段]
        PP_x, PP_y = np.meshgrid(P_x,P_y)
        WW_x, WW_y = np.meshgrid(W_x,W_y)
        PP_x = PP_x.reshape([1,-1])
...
def MatrixFilling_opt_new_full_part1(mesh,rooftops):
    try:
        # 矩阵化的x0_m　[坐标，区间段]
        x0_m = np.array([meshcell[0] for meshcell in mesh]).transpose()
        # 矩阵化的ｌｍ　[区间段]
        ...
        ha_m = ha_m.transpose()
        # 标准高斯点
        W_x,P_x = getWeightAndPoint_S(3)
        W_y,P_y = getWeightAndPoint_S(3)
        # 存储每个矩形上的高斯点 [高斯点个数，坐标，区间段]
        PP_x, PP_y = np.meshgrid(P_x,P_y)
        WW_x, WW_y = np.meshgrid(W_x,W_y)
        PP_x = PP_x.reshape([1,-1])
    ...
    try:
        Int_1_R_pos_list = scipy.zeros((len(rooftops),len(mesh)),dtype=float) #阻抗矩阵
        Int_1_R_neg_list = scipy.zeros((len(rooftops),len(mesh)),dtype=float) #阻抗矩阵
        Int_x_RXw_pos_list = scipy.zeros((len(rooftops),len(mesh)),dtype=float) #阻抗矩阵
        Int_x_RXw_neg_list = scipy.zeros((len(rooftops),len(mesh)),dtype=float) #阻抗矩阵
    except Exception as e:
        print e
        raise
    ...
    for n,edge_n in enumerate(rooftops): # 遍历所有测试
        ...
        try:
            ...
            Int_1_R_pos_list[n,:] = Int_1_R_pos
            Int_1_R_neg_list[n,:] = Int_1_R_neg
            Int_x_RXw_pos_list[n,:] = Int_x_RXw_pos
            Int_x_RXw_neg_list[n,:] = Int_x_RXw_neg
            ...
            print e
            raise
    return np.array([Int_1_R_pos_list,Int_1_R_neg_list,Int_x_RXw_pos_list,Int_x_RXw_neg_list])     

def MatrixFilling_opt_new_full_part2(mesh,rooftops, pre_Data):
    try:
        # 矩阵化的ｌｍ　[区间段]
        l_m = np.array([meshcell[2] for meshcell in mesh])
        # 矩阵化的ａ_m　[区间段]
    ...
    for n,edge_n in enumerate(rooftops): # 遍历所有测试
        ...
        try:
            ...
            temp1_neg = 1.+jK*DDist0_neg
            # 读取======================
            Int_1_R_pos = pre_Data[0,n,:]# 0.5l_m 和0.5a_m是高斯积分权重的缩放因子
            Int_1_R_neg = pre_Data[1,n,:]            
            Int_1_R_pos = np.add( np.multiply(Int_1_R_pos, temp1_pos), jK_lm_am)
            Int_1_R_neg = np.add( np.multiply(Int_1_R_neg, temp1_neg), jK_lm_am)            
            Int_ejkR_R_pos = np.multiply(Int_1_R_pos, ejkR0_pos)
            Int_ejkR_R_neg = np.multiply(Int_1_R_neg, ejkR0_neg)
            # 计算x_ejkR_RXw　用于＋矩形
            # 读取======================
            Int_x_RXw_pos = pre_Data[2,n,:]
            Int_x_RXw_neg = pre_Data[3,n,:]               
            Int_x_RXw_pos = np.add( np.multiply(Int_x_RXw_pos, temp1_pos), jK_lm2_am_half )
            Int_x_RXw_neg = np.add( np.multiply(Int_x_RXw_neg, temp1_neg), jK_lm2_am_half )
            Int_x_ejkR_RXw_pos = np.multiply(Int_x_RXw_pos, ejkR0_pos)
            Int_x_ejkR_RXw_neg = np.multiply(Int_x_RXw_neg, ejkR0_neg)
...
#    Z = MatrixFilling_opt_new_full(mesh,rooftops)
    pre_data = MatrixFilling_opt_new_full_part1(mesh,rooftops)
    Z = MatrixFilling_opt_new_full_part2(mesh,rooftops,pre_data)
...
#    plt.show()
    ...
    ths_phs = [(xx,np.pi*0.5) for xx in ths]
